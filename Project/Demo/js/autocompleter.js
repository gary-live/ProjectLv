    ...
    var methodOpt = {
        ...
        _init: function (opts) {
            ...
            for (var i = 0, count = $items.length; i < count; i++) {
                methodOpt._build($items.eq(i), opts);
            }
        ...
        _build: function ($node, opts) {
            if (!$node.hasClass("autocompleter-node")) {
                ...
                data.$autocompleter.on("mousedown.autocompleter", ".autocompleter-item", data, methodOpt._select)
                    .data("autocompleter", data);

                // Bind node events
                data.$node.on("keyup.autocompleter", data, methodOpt._onKeyup)
                    .on("keydown.autocompleter", data, methodOpt._onKeydownHelper)
                    .on("focus.autocompleter", data, methodOpt._onFocus)
                    .on("blur.autocompleter", data, methodOpt._onBlur)
                    .on("mousedown.autocompleter", data, methodOpt._onMousedown);
            }
        },
        ...
        _launch: function (data) {
            ...
            if (!data.empty && data.query.length === 0) {
                ...
                if (typeof data.source === 'object') {
                    ...
                    // Local search
                    var search = methodOpt._search(data.query, methodOpt._clone(data.source), data.limit);
                    if (search.length) {
                        methodOpt._response(search, data);
                    }
                } else {
                    if (data.jqxhr) {
                        data.jqxhr.abort();
                    ...
                    data.jqxhr = $.ajax({
                            ...
                            beforeSend: function (xhr) {
                                ...
                                methodOpt._clear(data);
                                if (data.cache) {
                                    var stored = methodOpt._getCache(this.url);
                                    if (stored) {
                                        xhr.abort();
                                        methodOpt._response(stored, data);
                                    }
                                }
                            }
                        })
                        .done(function (response) {
                            ...
                            if (data.offset) {
                                response = methodOpt._grab(response, data.offset);
                            }
                            if (data.cache) {
                                methodOpt._setCache(this.url, response);
                            }
                            methodOpt._response(response, data);
                        })
                        .always(function () {
        ...
        _response: function (response, data) {
            methodOpt._buildList(response, data);
        ...
        _open: function (e, instanceData) {
            ...
            if (!data.$node.prop("disabled") && !data.$autocompleter.hasClass("autocompleter-show") && data.$list && data.$list.length) {
                ...
                $body.on("click.autocompleter-" + data.guid, ":not(.autocompleter-item)", data, methodOpt._closeHelper);
            }
        },
